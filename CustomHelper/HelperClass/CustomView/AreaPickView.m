...
-(void)setUI
{
    ...
    UIView * bottomView = [UIFactory initViewWithFrame:CGRectMake(0, self.bounds.size.height/2, self.bounds.size.width, self.bounds.size.height/2) color:[UIColor whiteColor]];
    ...
    UIView * btnView = [UIFactory initViewWithFrame:CGRectMake(0, self.bounds.size.height/ 2 - 50, self.bounds.size.width, 50) color:[UIColor whiteColor]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(PickState == ProvinceState)
    {
        DistrictModel * model = _provinceArr[indexPath.row];
        cell.textLabel.text = model.name;

    }else if(PickState == CityState)
    {
        DistrictModel * model = self.cityArr[indexPath.row];
        cell.textLabel.text = model.name;
    }else {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(PickState == ProvinceState) {
        ...
        if (_selectProvinceCityAreaCall) {
            _selectProvinceCityAreaCall(selectedProvince,selectedCity,model.name,model.ID);
        }
...
//    [viewModel getDataSourceWithParamer:paramter suceess:^(NSArray *dataArr) {
